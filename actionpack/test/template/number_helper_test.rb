...
class NumberHelperTest < ActionView::TestCase
  ...
  def test_number_to_human_size
    ...
    assert_equal '1030 TB',   number_to_human_size((1026 * 1000**4))
    assert_equal '444 KB',    number_to_human_size(444 * 1000)
    assert_equal '999 MB',   number_to_human_size(999 * 1000**2)
    assert_equal '3 TB',      number_to_human_size(3 * 1000**4)
    ...
    assert_equal '1.01 KB',   number_to_human_size(1.0100 * 1000, :precision => 4)
    assert_equal '10 KB',   number_to_human_size(10.000 * 1000, :precision => 4)
    assert_equal '1 Byte',   number_to_human_size(1.1)
    assert_equal '10 Bytes', number_to_human_size(10)
  end

  def test_number_to_human_size_with_iec_prefix
    assert_equal '0 Bytes',   number_to_human_size(0, :prefix => :iec)
    assert_equal '1 Byte',    number_to_human_size(1, :prefix => :iec)
    ...
    assert_equal '1030 TiB',   number_to_human_size(terabytes(1026), :prefix => :iec)
    assert_equal '444 KiB',    number_to_human_size(kilobytes(444), :prefix => :iec)
    assert_equal '1020 MiB',   number_to_human_size(megabytes(1023), :prefix => :iec)
    assert_equal '3 TiB',      number_to_human_size(terabytes(3), :prefix => :iec)
    assert_equal '1.2 MiB',   number_to_human_size(1234567, :precision => 2, :prefix => :iec)
    assert_equal '3 Bytes',   number_to_human_size(3.14159265, :precision => 4, :prefix => :iec)
    assert_equal '123 Bytes', number_to_human_size('123', :prefix => :iec)
    assert_equal '1 KiB',   number_to_human_size(kilobytes(1.0123), :precision => 2, :prefix => :iec)
    assert_equal '1.01 KiB',   number_to_human_size(kilobytes(1.0100), :precision => 4, :prefix => :iec)
    assert_equal '10 KiB',   number_to_human_size(kilobytes(10.000), :precision => 4, :prefix => :iec)
    assert_equal '1 Byte',   number_to_human_size(1.1, :prefix => :iec)
    assert_equal '10 Bytes', number_to_human_size(10, :prefix => :iec)
  end
  ...
  def test_number_to_human_size_with_options_hash
    ...
    assert_equal '500 MB', number_to_human_size(500499000, :precision=>3)
    assert_equal '10 MB', number_to_human_size(9961472, :precision=>0)
    assert_equal '40 KB', number_to_human_size(41010, :precision => 1)
    assert_equal '40 KB', number_to_human_size(40499, :precision => 2)
    assert_equal '1.0 KB',   number_to_human_size(kilobytes(1.0123), :precision => 2, :strip_insignificant_zeros => false)
    assert_equal '1.012 KB',   number_to_human_size(1.0123 * 1000, :precision => 3, :significant => false)
    assert_equal '1 KB',   number_to_human_size(1.0123 * 1000, :precision => 0, :significant => true) #ignores significant it precision is 0
    assert_equal '9&lt;script&gt;&lt;/script&gt;86 KiB', number_to_human_size(10100, :separator => "<script></script>", :prefix => :iec)
  end

  def test_number_to_human_size_with_custom_delimiter_and_separator
    assert_equal '1,01 KB',     number_to_human_size(1.0123 * 1000, :precision => 3, :separator => ',')
    assert_equal '1,01 KB',     number_to_human_size(1.0100 * 1000, :precision => 4, :separator => ',')
    assert_equal '1.000,1 TB',  number_to_human_size(1000.1 * 1000**4, :precision => 5, :delimiter => '.', :separator => ',')
  end
