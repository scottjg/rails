...
class NumericExtFormattingTest < ActiveSupport::TestCase
  ...
  def test_to_s__human_size
    ...
    assert_equal '1.23 KB',   1234.to_s(:human_size)
    assert_equal '12.3 KB',   12345.to_s(:human_size)
    assert_equal '1.23 MB',   1234567.to_s(:human_size)
    assert_equal '1.23 GB',   1234567890.to_s(:human_size)
    assert_equal '1.23 TB',   1234567890123.to_s(:human_size)
    assert_equal '1030 TB',   (1026 * 1000**4).to_s(:human_size)
    assert_equal '444 KB',    (444 * 1000).to_s(:human_size)
    assert_equal '999 MB',    (999 * 1000**2).to_s(:human_size)
    assert_equal '3 TB',      (3 * 1000**4).to_s(:human_size)
    assert_equal '1.2 MB',    1234567.to_s(:human_size, :precision => 2)
    assert_equal '3 Bytes',   3.14159265.to_s(:human_size, :precision => 4)
    assert_equal '1 KB',      (1.0123 * 1000).to_s(:human_size, :precision => 2)
    assert_equal '1.01 KB',   (1.0100 * 1000).to_s(:human_size, :precision => 4)
    assert_equal '10 KB',     (10.000 * 1000).to_s(:human_size, :precision => 4)
    assert_equal '1 Byte',    1.1.to_s(:human_size)
    assert_equal '10 Bytes',  10.to_s(:human_size)
  end

  def test_to_s__human_size_with_iec_prefix
    assert_equal '0 Bytes',   0.to_s(:human_size, :prefix => :iec)
    assert_equal '1 Byte',    1.to_s(:human_size, :prefix => :iec)
    ...
    assert_equal '1030 TiB',   terabytes(1026).to_s(:human_size, :prefix => :iec)
    assert_equal '444 KiB',    kilobytes(444).to_s(:human_size, :prefix => :iec)
    assert_equal '1020 MiB',   megabytes(1023).to_s(:human_size, :prefix => :iec)
    assert_equal '3 TiB',      terabytes(3).to_s(:human_size, :prefix => :iec)
    assert_equal '1.2 MiB',    1234567.to_s(:human_size, :precision => 2, :prefix => :iec)
    assert_equal '3 Bytes',   3.14159265.to_s(:human_size, :precision => 4, :prefix => :iec)
    assert_equal '1 KiB',      kilobytes(1.0123).to_s(:human_size, :precision => 2, :prefix => :iec)
    assert_equal '1.01 KiB',   kilobytes(1.0100).to_s(:human_size, :precision => 4, :prefix => :iec)
    assert_equal '10 KiB',     kilobytes(10.000).to_s(:human_size, :precision => 4, :prefix => :iec)
    assert_equal '1 Byte',    1.1.to_s(:human_size, :prefix => :iec)
    assert_equal '10 Bytes',  10.to_s(:human_size, :prefix => :iec)
  end
  ...
  def test_to_s__human_size_with_options_hash
    ...
    assert_equal '500 MB',   500499000.to_s(:human_size, :precision=>3)
    assert_equal '10 MB',    9961472.to_s(:human_size, :precision=>0)
    assert_equal '40 KB',    41010.to_s(:human_size, :precision => 1)
    assert_equal '40 KB',    40499.to_s(:human_size, :precision => 2)
    assert_equal '1.0 KB',   (1.0123 * 1000).to_s(:human_size, :precision => 2, :strip_insignificant_zeros => false)
    assert_equal '1.012 KB', (1.0123 * 1000).to_s(:human_size, :precision => 3, :significant => false)
    assert_equal '1 KB',     (1.0123 * 1000).to_s(:human_size, :precision => 0, :significant => true) #ignores significant it precision is 0
  end
  
  def test_to_s__human_size_with_custom_delimiter_and_separator
    assert_equal '1,01 KB',     (1.0123 * 1000).to_s(:human_size, :precision => 3, :separator => ',')
    assert_equal '1,01 KB',     (1.0100 * 1000).to_s(:human_size, :precision => 4, :separator => ',')
    assert_equal '1.000,1 TB',  (1000.1 * 1000**4).to_s(:human_size, :precision => 5, :delimiter => '.', :separator => ',')
  end
