...
# thanks to Masao's String extensions these should work the same in Ruby 1.8 (patched) and Ruby 1.9 (native)
# some tests were taken from Masao's tests: http://github.com/mutoh/gettext/blob/edbbe1fa8238fa12c7f26f2418403015f0270e47/test/test_string.rb
class StringInterpolationTest < ActiveSupport::TestCase
  test "interpolates a single argument" do
    assert_equal "Masao", "%s" % "Masao"
  end

  test "interpolates an array argument" do
    assert_equal "1 message", "%d %s" % [1, 'message']
  end

  test "interpolates a hash argument w/ named placeholders" do
    assert_equal "Masao Mutoh", "%{first} %{last}" % { :first => 'Masao', :last => 'Mutoh' }
  end

  test "interpolates a hash argument w/ named placeholders (reverse order)" do
    assert_equal "Mutoh, Masao", "%{last}, %{first}" % { :first => 'Masao', :last => 'Mutoh' }
  end

  test "interpolates named placeholders with sprintf syntax" do
    assert_equal "10, 43.4", "%<integer>d, %<float>.1f" % { :integer => 10, :float => 43.4 }
  end

  test "interpolates named placeholders with sprintf syntax, does not recurse" do
    assert_equal "%<not_translated>s", "%{msg}" % { :msg => '%<not_translated>s', :not_translated => 'should not happen' }
  end

  test "interpolation does not replace anything when no placeholders are given (given a Hash)" do
    assert_equal("aaa", "aaa" % { :num => 1 })
  end

  test "interpolation does not replace anything when no placeholders are given (given an Array)" do
    silence_warnings { assert_equal("bbb", "bbb" % [1]) }
  end

  test "interpolation sprintf behaviour equals Ruby 1.9 behaviour" do
  ...
  test "interpolation old-style sprintf still works" do
    assert_equal("foo 1.000000", "%s %f" % ["foo", 1.0])
  end

  test "interpolation raises an ArgumentError when the string has extra placeholders (Array)" do
    assert_raise(ArgumentError) do # Ruby 1.9 msg: "too few arguments"
      "%s %s" % %w(Masao)
    end
  end

  test "interpolation raises a KeyError when the string has extra placeholders (Hash)" do
    assert_raise(KeyError) do # Ruby 1.9 msg: "key not found"
      "%{first} %{last}" % { :first => 'Masao' }
    end
  end

  test "interpolation does not raise when passed extra values (Array)" do
    assert_nothing_raised do
      assert_equal "Masao", silence_warnings { "%s" % %w(Masao Mutoh) }
    end
  end

  test "interpolation does not raise when passed extra values (Hash)" do
    assert_nothing_raised do
      assert_equal "Masao Mutoh", "%{first} %{last}" % { :first => 'Masao', :last => 'Mutoh', :salutation => 'Mr.' }
    end
  end

  test "% acts as escape character in interpolation" do
