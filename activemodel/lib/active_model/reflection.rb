module ActiveModel
  module Reflection # :nodoc:
    ...
    # Reflection allows you to interrogate Active Model classes and objects about their associations and aggregations.
    # This information can, for example, be used in a form builder that took an Active Model object and created input
    # fields for all of the attributes depending on their type and displayed the associations to other objects.
    #
    # You can find the interface for the AggregateReflection and AssociationReflection classes in the abstract MacroReflection class.
    module ClassMethods
      def create_reflection(macro, name, options, active_model)
        case macro
          when :has_many, :belongs_to, :has_one, :has_and_belongs_to_many
            reflection = AssociationReflection.new(macro, name, options, active_model)
          when :composed_of
            reflection = AggregateReflection.new(macro, name, options, active_model)
        end
    ...
    class MacroReflection
      ...
      def initialize(macro, name, options, active_model)
        @macro, @name, @options, @active_model = macro, name, options, active_model
      end
      ...
      # Returns +true+ if +self+ and +other_aggregation+ have the same +name+ attribute, +active_model+ attribute,
      # and +other_aggregation+ has an options hash assigned to it.
      def ==(other_aggregation)
        other_aggregation.kind_of?(self.class) && name == other_aggregation.name && other_aggregation.options && active_model == other_aggregation.active_model
      end
    ...
    # Holds all the meta-data about an aggregation as it was specified in the Active Model class.
    class AggregateReflection < MacroReflection #:nodoc:
    end

    # Holds all the meta-data about an association as it was specified in the Active Model class.
    class AssociationReflection < MacroReflection #:nodoc:
      ...
      def klass
        @klass ||= active_model.send(:compute_type, class_name)
      end
      ...
      # with ActiveModel::Base#save. +options+ will be passed to the class's
      ...
      # Creates a new instance of the associated class, and immediates saves it
      # with ActiveModel::Base#save!. +options+ will be passed to the class's
      # creation method. If the created model doesn't pass validations, then an
      ...
      def through_reflection
        @through_reflection ||= options[:through] ? active_model.reflect_on_association(options[:through]) : false
      end
      ...
      def check_validity!
        if options[:through]
          if through_reflection.nil?
            raise HasManyThroughAssociationNotFoundError.new(active_model.name, self)
          end
          ...
          if options[:source_type] && source_reflection.options[:polymorphic].nil?
            raise HasManyThroughAssociationPointlessSourceTypeError.new(active_model.name, self, source_reflection)
          end
          
          if source_reflection.options[:polymorphic] && options[:source_type].nil?
            raise HasManyThroughAssociationPolymorphicError.new(active_model.name, self, source_reflection)
          end
