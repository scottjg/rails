  ...
  namespace :structure do
    ...
    task :dump => :environment do
      ...
      when /postgresql/
        ...
        user_option = abcs[Rails.env]['username'] ? "-U #{abcs[Rails.env]['username']} " : ""
        `pg_dump -i #{user_option} -s -x -O -f db/#{Rails.env}_structure.sql #{search_path} #{abcs[Rails.env]['database']}`
  ...
  namespace :test do
    ...
    task :clone_structure => [ 'db:structure:dump', 'db:test:purge' ] do
      ...
      when /postgresql/
        ...
        user_option = abcs[Rails.env]['username'] ? "-U #{abcs[Rails.env]['username']} " : ""
        `psql #{user_option} -f #{Rails.root}/db/#{Rails.env}_structure.sql #{abcs['test']['database']} #{abcs['test']['template']}`
      when /sqlite/
