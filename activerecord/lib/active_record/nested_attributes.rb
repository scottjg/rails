    ...
    def assign_nested_attributes_for_one_to_one_association(association_name, attributes)
      ...
      if record && options[:update_only]
        unless call_reject_if(association_name, attributes)
          assign_to_or_mark_for_destruction(record, attributes.except(pk_name), options[:allow_destroy])
        end
        
      elsif record && (attributes[pk_name].present? && attributes[pk_name].to_s == record.id.to_s)
        unless call_reject_if(association_name, attributes)
          assign_to_or_mark_for_destruction(record, attributes.except(pk_name), options[:allow_destroy])
        end
