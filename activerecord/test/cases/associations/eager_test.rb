...
class EagerAssociationTest < ActiveRecord::TestCase
  ...
  def test_associations_loaded_for_all_records_when_summing_with_block
    post = Post.create!(:title => 'foo', :body => "I like cars!")
    first_category = Category.create! :name => 'First!', :posts => [post]
    second_category = Category.create! :name => 'Second!', :posts => [post]

    categories = Category.where(:id => [first_category.id, second_category.id]).includes(:posts)
    assert_queries(2) do
      categories.each do |category|
        category.posts.sum(&:id)
      end
    end
  end
