...
class NamedScopeTest < ActiveRecord::TestCase
  ...
  def test_chaining_should_use_latest_conditions_when_searching
    ...
    assert_equal Topic.where(:approved => true).to_a, Topic.rejected_hash.approved_hash.all
    assert_equal Topic.where(:approved => false).to_a, Topic.approved_hash.rejected_hash.all

    # Nested hash conditions with same keys
    assert_equal [posts(:sti_comments)], Post.with_special_comments_hash.with_very_special_comments_hash.all

    # Nested hash conditions with different keys
    assert_equal [posts(:sti_comments)], Post.with_special_comments_hash.with_post(4).all.uniq
  end
  ...
  def test_scopes_are_deprecated
    assert_deprecated { Post.scope :asdashsdfsd }
  end
end
