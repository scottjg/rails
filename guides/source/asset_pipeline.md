...
:source_digests:
  ...
  application.js.coffee: a9844320226b169f67246c9280d7a882
  application.css.scss: 4bfde1ead6cbafb32fca2031731070f0
...
The asset pipeline will now only recompile assets that have changed. When deploying with Capistrano,
this means that you should copy the assets from your previous deploy into your current release's `public/assets` folder before running the `assets:precompile` task. This will greatly speed up your deployment if no assets have changed.

#### Automatic clean up of old assets

By default, the `rake assets:precompile` task now removes out-of-date assets after it has compiled a new version of that asset. This will only happen if you haven't disabled asset fingerprinting.
If you don't want to automatically delete old assets, set `config.assets.clean_after_precompile = false` in `config/environments/production.rb`.

NOTE: Do not symlink `public/assets` to a shared assets folder. Keeping a separate copy of `public/assets` for each release is necessary, because shared assets would make it impossible to roll back to a previous release.
